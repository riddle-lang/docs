...
例如，在 Windows x86_64 上使用 `riddle-v0.3.0-windows-x86_64.zip` 安装工具；要构建 `aarch64-unknown-linux-gnu` 程序，则运行 `ridup target add aarch64-unknown-linux-gnu` 安装对应目标组件。目标组件不会替代 Linux sysroot、Windows SDK/MSVC 库或 Apple SDK，也不需要单独加入 `PATH`。
