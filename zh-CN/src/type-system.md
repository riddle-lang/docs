...
函数值在运行时由调用地址和可选闭包环境组成。非捕获匿名函数和命名函数使用空环境；捕获匿名函数的环境保存共享引用、可变引用或移入的值，因此从函数返回的闭包仍能安全访问捕获内容。编译器根据捕获方式推断 `Fn`、`FnMut` 或 `FnOnce` 调用能力，这些能力不改变 `fun(A) -> R` 的表面类型语法。
...
| `fun(A) -> R` | `{ call, env }` 闭包结构；`call` 接收隐藏环境参数 |
