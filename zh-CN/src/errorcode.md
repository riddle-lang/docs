...
<a id="e0011"></a>
### E0011 — 未知字面量后缀
整数或浮点数字面量的类型后缀无效。词法器会接受 `i128`、`u128`、`f16`、`f128` 后缀，但类型系统只支持 8 到 64 位整数与 `f32` / `f64`，使用这些后缀也会触发本错误。
```riddle
let x = 42i128;  // E0011: unknown integer literal suffix `i128`
let y = 3.5f16;  // E0011: unknown float literal suffix `f16`
```
